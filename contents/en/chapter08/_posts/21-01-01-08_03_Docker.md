...
## From a manual box to a managed runtime

Docker is not the end of the deploy story. Treat it as one rung on a ladder IUH pairs should be able to *name*:

```text
SSH + apt on one VM
        → bake an image / cloud-init once
        → Docker Compose on a VM you still patch
        → managed containers (ECS, ACA, Cloud Run, GKE Autopilot, …)
        → functions / scale-to-zero (Chapter 08 serverless notes)
```

**Manual** means you still own the guest OS, the daemon, the disk, and the 3 a.m. `apt upgrade`. **Managed** means you still own the *image contract* (Dockerfile, user, healthcheck, stdout logs) but the provider owns the nodes—or there are no nodes in your mental model at all.

A Course Board pair should start on the Compose rung ([Deploy Track Lab A]({{ site.baseurl }}{% multilang_post_url contents/chapter15/21-01-01-15_01_Lab_Docker_Local %})). Move up a rung only when a learning outcome requires it (a hostname on the internet, an autoscaler you did not write). Moving *down* a rung to “just SSH” is fine for debugging; staying there for the capstone usually means you skipped the network and rollback story in Chapters 01-07 and 13-04.

<div class="content-box insight-box">
<p><strong>The image is the portable artifact.</strong> Managed platforms still pull a digest. If you cannot point at <code>docker build</code> output, you are not ready for a platform that hides the VM.</p>
</div>
